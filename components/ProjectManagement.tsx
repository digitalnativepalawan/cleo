...
import ProjectModule from './portal/modules/VincenteHouseModule.tsx'; // <-- explicit .tsx extension
  ...
  return (
    <div className="h-full bg-[var(--bg-secondary)] flex flex-col">
      ...
      <div className="flex flex-1 md:grid md:grid-cols-[256px_1fr] min-h-0 relative">
        ...
        {/* Sidebar */}
        <aside className={`absolute top-0 left-0 h-full w-64 flex-shrink-0 bg-[var(--bg-primary)] border-r border-[var(--border-primary)] flex flex-col transform transition-transform duration-300 ease-in-out z-40 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <nav className="flex-1 overflow-y-auto p-2 space-y-1">
            {INITIAL_PROJECTS.map(project => (
              <button
                ...
                className={`w-full text-left flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors ${activeProjectId === project.id ? 'bg-blue-50 text-[var(--accent-primary)]' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}
              >
