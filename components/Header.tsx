                ...
                <div className="flex flex-col h-full">
                    ...
                    <nav className="flex-grow p-4">
                        <ul className="space-y-2">
                            {navItems.map((item) => (
                                <li key={item.name}>
                                    <a href={item.href} onClick={onClose} className="block w-full text-lg font-medium text-gray-700 hover:bg-gray-100 p-3 rounded-md transition-colors">
                                        {item.name}
                                    </a>
                                </li>
                            ))}
                    ...
                    <div className="p-4 border-t">
                        <h4 className="text-xs font-semibold text-gray-500 mb-3 uppercase tracking-wider">Currency</h4>
                         <div className="flex space-x-2">
                            {currencies.map(currency => (
                                <button
                                    key={currency}
                                    onClick={() => setSelectedCurrency(currency)}
                                    className={`flex-1 py-2 text-sm rounded-md border transition-colors ${selectedCurrency === currency ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
                                >
                                    {currency}
                                </button>
                            ))}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 h-full">
                    <div className="flex items-center justify-between h-full">
                        ...
                        <div className="flex-shrink-0 flex flex-col items-center">
                            <div className="font-mono tracking-wider text-3xl md:text-4xl text-gray-900 flex items-baseline" aria-live="polite">
                            ...
                            <div className="font-serif text-xs md:text-sm text-gray-600 mt-1" aria-label={`Date is ${formattedDate}`}>
                                {formattedDate}
                            </div>
                        </div>
                        ...
                        <div className="flex-1 flex justify-end items-center space-x-2 sm:space-x-4">
                            ...
                            <div className="hidden md:block">
                                <button 
                                    ...
                                    aria-label="Blog"
                                    className="flex items-center space-x-2 border border-gray-300 hover:bg-gray-100 px-3 py-1.5 text-sm font-medium text-gray-700 rounded-full transition-all duration-200"
                                >
                            ...
                            <div className="hidden md:block" ref={dropdownRef}>
                                <div className="relative">
                                     <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} aria-haspopup="true" aria-expanded={isDropdownOpen} className="flex items-center space-x-1 border border-gray-300 hover:bg-gray-100 px-3 py-1.5 text-sm font-medium text-gray-700 rounded-full transition-all duration-200">
